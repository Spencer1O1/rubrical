...
func threeBandRubric() RubricContext {
	return RubricContext{Rows: []RubricRow{{
...
func TestApplyRubricScoring_excellentMostlyMet(t *testing.T) {
	...
	scored, err := ApplyRubricScoring(resp, threeBandRubric())
	if err != nil {
		t.Fatal(err)
	}
	c := scored.Criteria[0]
...
func TestApplyRubricScoring_rejectsUnknownBand(t *testing.T) {
	resp := &schema.ProviderResponse{Criteria: []schema.CriterionAssessment{{
		CriterionName:  "General Overview",
		SelectedRating: "Outstanding",
		BandPosition:   80,
	}}}
	_, err := ApplyRubricScoring(resp, threeBandRubric())
	if err == nil {
